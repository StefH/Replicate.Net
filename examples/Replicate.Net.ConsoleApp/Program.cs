...
async Task RunOnPredictionsAsync()
{
    ...
    var model = await client.GetModelAsync("cjwbw", "stable-diffusion-high-resolution").ConfigureAwait(false);
    Console.WriteLine(JsonConvert.SerializeObject(model, Formatting.Indented));

    var modelVersions = await client.GetAllModelVersionsAsync("cjwbw", "stable-diffusion-high-resolution").ConfigureAwait(false);
    ...
    var response2 = await client.CreatePredictionAndWaitOnResultAsync(requestcjwbw).ConfigureAwait(false);

    Console.WriteLine(JsonConvert.SerializeObject(response2, Formatting.Indented));
}